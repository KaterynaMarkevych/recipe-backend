...
const FilterSection = ({ onFilterChange }) => {
  ...
  const dietTypes = ["Вегетеріанська", "Веганська", "Безглютенова", "Безлактозна"];
  const cookingTimes = [
  ...
  return (
    <div className="bg-gray-50 p-4 rounded-lg mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {/* Тип страви */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Тип страви</label>
          <select
        ...
        {/* Час приготування */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Час приготування</label>
          <select
            value={filters.cookingTime || ''}
            onChange={(e) => handleFilterChange('cookingTime', e.target.value || null)}
            className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
          >
        ...
        {/* Пошук */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Пошук рецептів</label>
          <input
            type="text"
      ...
      <div className="mt-4">
        <label className="block text-sm font-medium text-gray-700 mb-2">Дієтичні особливості</label>
        <div className="flex flex-wrap gap-2">
          {dietTypes.map((diet) => {
            const isSelected = filters.diet && filters.diet.split(',').includes(diet);
            return (
              <button
                key={diet}
